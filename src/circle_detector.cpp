...
const int low_r=30, low_g=30, low_b=30;
const int high_r=100, high_g=100, high_b=100;
...
int main(int argc, char *argv[])
{
  ...
  while(1)
    ...
    //**************** Find circles in the image ****************************
        ...
        // Mask for tracking
        //cv::inRange(gray_image, cv::Scalar(low_b,low_g,low_r), cv::Scalar(high_b,high_g,high_r), frame_threshold);
        //cv::erode(frame_threshold, frame_threshold, 2);
        //cv::dilate( frame_threshold, frame_threshold, 2 );
        ...
        for(unsigned int ii = 0; ii < circles.size(); ii++ )
        {
            if ( circles[ii][0] != -1 )
            {
                    ...
                    cv::circle(image, center, 5, cv::Scalar(128, 255, 0), -1, 8, 0 );// circle center in green
                    ...
                    cv::putText(image, stringStream.str(), center, cv::FONT_HERSHEY_PLAIN, 0.8, cv::Scalar(0, 255, 0), 2, 0.5);
            }
        }
    ...
    //********************************************************************
        ...
        //show image
        cv::imshow("Hough Circles Transformation", image);

        //cv::imshow("Hough Circles Transformation", frame_threshold);
